...
# Use shell arithmetic for calculation
SECONDS=$((MINUTES * 60))
nohup bash -c "sleep $SECONDS && tmux send-keys -t $TARGET 'Time for orchestrator check! cat \"$SCRIPT_DIR/next_check_note.txt\" && python3 \"$SCRIPT_DIR/qwen_control.py\" status detailed' && sleep 1 && tmux send-keys -t $TARGET Enter" > /dev/null 2>&1 &
